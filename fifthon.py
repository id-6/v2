...
ownerhson_id = 5841353971
@fifthon.on(events.NewMessage(outgoing=False, pattern='بيثون'))
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id == ownerhson_id :
        order = await event.reply('● ● ● @F_P_l ● ● ●')


@fifthon.on(events.NewMessage(outgoing=True, pattern=r"\.اعادة تشغيل"))
async def update(event):
    await event.edit(" ثـواني ويشتغل ؟ لمن مايشتغل طب سولف بكروبي  @Bethon2 ")
